...
class TaskStage(Enum):
    """Stages of the pick-and-place task."""
    APPROACHING_OBJECT = 1
    GRASPING = 2
    MOVING_TO_TARGET = 3
    RELEASING = 4
    COMPLETE = 5
...
class MotionPlanningPolicy(Policy):
    ...
    def __init__(
        ...
        self.use_target_fixture: bool = False  # Set to True if target_fixture_pos is available
    ...
    def get_action(self, time: Optional[float] = None) -> Dict[str, Any]:
        ...
        if not self.is_active:
            ...
            # Generate wrist trajectories based on current stage
            left_wrist_matrix = self.latest_left_wrist_data
            right_wrist_matrix = self._compute_stage_trajectory()
            self.latest_right_wrist_data = right_wrist_matrix
            
            # Update gripper state based on current stage
...
based on task stage."""
        ...
        self.trajectory_start_time = time_module.monotonic()
        self.stage_start_time = time_module.monotonic()
        ...
        if self.use_target_fixture and "target_fixture_pos" in self.observation:
            fixture_pos_world = np.array(self.observation["target_fixture_pos"])
            if base_pose is not None:
                self.target_fixture_pos = base_rot.inv().apply(fixture_pos_world - base_pos)
                print(f"Target fixture detected at: {self.target_fixture_pos}")
            else:
    ...
    def activate_policy(self, wait_for_activation: int = 5):
        ...
        elapsed = time_module.monotonic() - self.stage_start_time
        
        if self.current_stage == TaskStage.APPROACHING_OBJECT:
            if elapsed >= self.trajectory_duration:
                print(f"Stage complete: APPROACHING_OBJECT -> GRASPING")
                self.current_stage = TaskStage.GRASPING
                self.stage_start_time = time_module.monotonic()
                
        elif self.current_stage == TaskStage.GRASPING:
            if elapsed >= self.grasp_duration:
                if self.use_target_fixture and self.target_fixture_pos is not None:
                    print(f"Stage complete: GRASPING -> MOVING_TO_TARGET")
                    self.current_stage = TaskStage.MOVING_TO_TARGET
                    self.stage_start_time = time_module.monotonic()
                    self._initialize_target_fixture_trajectory()
                else:
                    print(f"Stage complete: GRASPING -> COMPLETE (no target fixture)")
                    self.current_stage = TaskStage.COMPLETE
                    
        elif self.current_stage == TaskStage.MOVING_TO_TARGET:
            if elapsed >= self.trajectory_duration:
                print(f"Stage complete: MOVING_TO_TARGET -> RELEASING")
                self.current_stage = TaskStage.RELEASING
                self.stage_start_time = time_module.monotonic()
                
        elif self.current_stage == TaskStage.RELEASING:
            if elapsed >= self.release_duration:
                print(f"Stage complete: RELEASING -> COMPLETE")
                self.current_stage = TaskStage.COMPLETE
                
        elif self.current_stage == TaskStage.COMPLETE:
            pass  # Stay in complete state
    
    def _initialize_target_fixture_trajectory(self):
        """Initialize trajectory from current position to target fixture."""
        if self.target_fixture_pos is None:
            return
            
        # Start from current wrist pose
        self.start_wrist_pose = self.latest_right_wrist_data.copy()
        ...
        print(f"Initialized target fixture trajectory:")
        print(f"  Start position: {self.start_wrist_pose[:3, 3]}")
        print(f"  Target position: {target_pos}")
        print(f"  Duration: {self.trajectory_duration}s")
    
    def _compute_stage_trajectory(self) -> np.ndarray:
        """Compute wrist pose based on current task stage."""
        if self.current_stage == TaskStage.APPROACHING_OBJECT:
            elapsed = time_module.monotonic() - self.stage_start_time
            t = elapsed / self.trajectory_duration
            return self._compute_trajectory_point(t)
            
        elif self.current_stage == TaskStage.GRASPING:
            # Hold position at object
            return self.target_wrist_pose.copy()
            ...
            # Hold position at target fixture
            return self.target_wrist_pose.copy()
            
        elif self.current_stage == TaskStage.COMPLETE:
            # Hold final position
            return self.latest_right_wrist_data.copy()
    ...
    def _update_gripper_state(self):
        ...
        if self.current_stage == TaskStage.APPROACHING_OBJECT:
            ...
            elapsed = time_module.monotonic() - self.stage_start_time
            t = min(elapsed / self.grasp_duration, 1.0)
            self.current_right_hand_q = (1 - t) * self.open_gripper_q + t * self.closed_gripper_q
            
        elif self.current_stage == TaskStage.MOVING_TO_TARGET:
            # Keep gripper closed
            self.current_right_hand_q = self.closed_gripper_q.copy()
            
        elif self.current_stage == TaskStage.RELEASING:
            # Gradually open gripper
            elapsed = time_module.monotonic() - self.stage_start_time
            t = min(elapsed / self.release_duration, 1.0)
            self.current_right_hand_q = (1 - t) * self.closed_gripper_q + t * self.open_gripper_q
